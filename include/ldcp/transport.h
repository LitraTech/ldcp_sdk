...
public:
  ...
  const static int SCAN_PACKET_LENGTH_MAX = 65535;
...
public:
  ...
  typedef std::function<void(std::vector<uint8_t>)> ReceivedScanPacketCallback;
...
public:
  ...
  virtual error_t connect(int timeout_ms) = 0;
  ...
  void setReceivedScanPacketCallback(ReceivedScanPacketCallback callback);
...
protected:
  ...
  ReceivedScanPacketCallback received_scan_packet_callback_;
};
